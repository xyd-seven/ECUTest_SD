...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  connect(btnConfigEditor, &QPushButton::clicked, this, [this]() {
    ...
    connect(&dlg, &ConfigEditorDialog::configSaved, this,
            &MainWindow::reloadConfigsAllChannels);
    dlg.exec(); // 模态阻塞弹出
  });
  ...
  connect(m_spinBoxCount, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &MainWindow::onChannelCountChanged);
  ...
  connect(m_scanProcessTimer, &QTimer::timeout, this,
          &MainWindow::processScanBuffer);
...
void MainWindow::onChannelCountChanged(int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    connect(w, &DeviceChannelWidget::barcodeReturnPressed, this,
            &MainWindow::onChannelScanFinished);
    connect(w, &DeviceChannelWidget::barcodeCleared, this,
...
void MainWindow::onChannelScanFinished(int currentId) {
  ...
  for (int j = 1; j < total; ++j) {
    ...
    if (m_channels[nextIdx]->isBarcodeEmpty()) {
      QMetaObject::invokeMethod(
          this, [=]() { m_channels[nextIdx]->setFocusToBarcode(); },
          Qt::QueuedConnection);
      break;
    }
  }
